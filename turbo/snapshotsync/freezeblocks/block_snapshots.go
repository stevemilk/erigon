...
type segments struct {
	// TODO@steven - when should we use RWMutex? divide lock into 2 locks?
	lock            sync.RWMutex
...
func (s *RoSnapshots) calculateVisibleSegments() {
	s.segments.Scan(func(segtype snaptype.Enum, value *segments) bool {
		...
		for i, sn := range dirtySegments {
			...
			maxIndex = i
			existNewVisible = true
		}

		if existNewVisible {
			value.visibleSegments = append(value.visibleSegments, dirtySegments[:maxIndex+1]...)
			value.dirtySegments = dirtySegments[maxIndex+1:]
		}
		return true
	})
}
...
func (s *RoSnapshots) idxAvailability() uint64 {
	...
	var maxIdx uint64
	s.segments.Scan(func(segtype snaptype.Enum, value *segments) bool {
		if len(value.visibleSegments) == 0 || !s.HasType(segtype.Type()) {
			return true
		}
		if !findIdx {
			...
			maxIdx = min(maxIdx, value.maxVisibleBlock.Load())
		}
		return true
	})
...
// TODO@steven - visible files may be deleted manually, so call recalc after rebuild
